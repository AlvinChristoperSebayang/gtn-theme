...
export default {
  methods: {
    ...
    guidGenerator() {
      ...
      extensionChecker(fileName){
        const allowedExtensions = [".png", ".svg", ".doc",".ppt",".pdf",".jpg",".xls",".zip",".rar"];
    ...
    addImageToList(inputfile){
      const inputFile = document.getElementById(inputfile);
      const invalidFiles = [];
      for (let file of inputFile.files) {
        if (this.extensionChecker(file.name)) {
          dt.items.add(file);
        }else{
          invalidFiles.push(file.name);
          const errorMessage = `Invalid File Extension: ${invalidFiles.join(', ')}`;
          Toastify({
        text: errorMessage,
        duration: 2000,
        gravity: "bottom", 
        position: "center",
        backgroundColor: "#de2d27", 
        className: "toastify-invalid", 
    }).showToast();
        }
      }
      inputFile.files = dt.files;
    },
    removeImageFromList(inputfile, index){
      const inputFile = document.getElementById(inputfile);
